...
<!-- <script>
  ...
  fetch('json/slider.json')
    .then(res => res.json())
    .then(data => {
      // Filter active slides and sort by sort_number ascending
      ...
      activeSlides.forEach(({ slider_image, slider_title, slider_description }) => {
        const slide = document.createElement('div');
        slide.className = 'swiper-slide relative';

        slide.innerHTML = `
          <img
            src="cloths/slides/${slider_image}"
            alt="${slider_title}"
          ...
          >
            <h2 class="text-3xl font-extrabold drop-shadow-lg mb-2">${slider_title}</h2>
            <p class="text-sm drop-shadow-md mb-4">${slider_description}</p>
            <a
              href="/shop"
